...
void updateGame() {
  ...
  // Camera scroll
  scrollX = lerp(scrollX, playerX - width/4, 0.1);
  ...
  // Goal check
  if (dist(playerX, playerY, goal.x, goal.y) < playerSize) {
    gameState = 2;
  }
}
...
void keyPressed() {
  ...
  if (key == ENTER) {
    ...
    else if (gameState == 2 || gameState == 3) initGame();
  }
}
